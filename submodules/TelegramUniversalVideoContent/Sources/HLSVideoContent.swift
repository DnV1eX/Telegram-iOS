...
private final class HLSVideoContentNode: ASDisplayNode, UniversalVideoContentNode {
    ...
    init(accountId: AccountRecordId, postbox: Postbox, audioSessionManager: ManagedAudioSession, userLocation: MediaResourceUserLocation, fileReference: FileMediaReference, streamVideo: Bool, loopVideo: Bool, enableSound: Bool, baseRate: Double, fetchAutomatically: Bool) {
        ...
        self.player = Players.default
        self.player.defaultRate = baseRate
        if !enableSound {
            self.player.mute()
        }
        
        self.playerNode = ASDisplayNode()
        self.playerNode.setLayerBlock { [player] in
            player.layer
        }
        ...
        self.player.onChangeStatus = { [weak self] in
            self?.updateStatus()
        }
        
        self.player.itemDidPlayToEndTime = { [weak self] in
        ...
        if let playerSource {
            self.serverDisposable = SharedHLSServer.shared.registerPlayer(source: playerSource) { [weak self] in
                Queue.mainQueue().async {
                    guard let self else {
                        return
                    }
    ...
    private func updateStatus() {
        ...
        if player.isBuffering {
            status = .buffering(initial: false, whilePlaying: player.isPlaying, progress: 0.0, display: true)
        } else {
    ...
    func setSoundMuted(soundMuted: Bool) {
        soundMuted ? self.player.mute() : self.player.unmute()
    }
    ...
    func setVideoQuality(_ videoQuality: UniversalVideoContentVideoQuality) {
        ...
        guard let currentItem = self.player.currentItem, let playerSource else {
            return
        }
